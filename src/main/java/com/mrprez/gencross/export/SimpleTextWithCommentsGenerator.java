...
public class SimpleTextWithCommentsGenerator extends FileGenerator {
	...
	private void writeProperty(Property property, OutputStream os, int depth) throws IOException{
		...
		if(property.getComment()!=null && !property.getComment().isEmpty()){
			os.write("\n".getBytes());
			for(int i=0; i<depth+2; i++){
				os.write("  ".getBytes());
			}
			os.write(("["+property.getComment()+"]").getBytes());
		}
		os.write("\n".getBytes());
